...
export const CreateReport = (data) => async (dispatch) => {
  try {
    ...
    dispatch({
      type: types.CREATE_REPORT_SUCCESS,
      payload: res.data, // Assuming response data is payload
    });
    return res.data; // Returning response data for further handling if needed
  } catch (error) {
    dispatch({
      type: types.CREATE_REPORT_ERROR,
      payload: {
        message: error.message, // Access error message
      },
    });
...
export const GetAmbulances = () => async (dispatch) => {
  try {
    dispatch({ type: types.GET_AMBULANCES_REQUEST });
    const res = await axios.get(
      `https://zany-gray-clam-gear.cyclic.app/ambulances`
    );
    console.log(res.data);
    dispatch({
      type: types.GET_AMBULANCES_SUCCESS,
      payload: res.data,
    });
  } catch (error) {
    console.log(error);
...
export const GetNurses = () => async (dispatch) => {
  try {
    dispatch({ type: types.GET_NURSES_REQUEST });
    const res = await axios.get(
      `https://zany-gray-clam-gear.cyclic.app/nurses`
    );
    console.log(res.data);
    dispatch({
